...
class DB:
    ...
    def update_path_when_finished(self) -> List[FlightPath]:
        """Updates finished and finished_at columns of flight_paths for records with no update for more then 30 min."""
        with self.conn.cursor() as curs:
            time_now = int(time.time())
            curs.execute(
                f"SELECT * from flight_paths WHERE finished = False AND %s - last_update > 1800", (time_now,)
            )
            updated_records = [FlightPath(*x) for x in curs.fetchall()]
            curs.execute(
    ...
    def update_callsigns(self, callsign, arrival_airport, departure_airport):
        with self.conn.cursor() as curs:
            ...
            record = CallsignMemo(*curs.fetchone())

            if record:
                if record[1] != arrival_airport or record[2] != departure_airport:
                    curs.execute(
